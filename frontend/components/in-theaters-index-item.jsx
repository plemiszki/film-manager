...
export default class InTheatersIndexItem extends React.Component {
  ...
  componentDidMount() {
    ...
    $(
      `.${hyphenatedSection} .top-drop-zone, .${hyphenatedSection} .drop-zone`,
    ).droppable({
  ...
  mouseDownHandler(e) {
    $(".handle, a, input, textarea, .x-button, .nice-select, tr").addClass(
      "grabbing",
    );
  ...
  mouseUpHandler(e) {
    $(
      ".handle, a, input, textarea, .x-button, .nice-select, tr, table",
  ...
  dropHandler(e, ui) {
    ...
    const newOrder = rearrangeFields({
      currentOrder,
      draggedIndex,
      dropZoneIndex,
    });
      ...
      <>
        <tr>
          <td
            id={"index-" + film.order}
            className="indent"
            data-index={film.order}
            data-section={section}
          >
            <div
              className={"top-drop-zone" + (film.order === 0 ? "" : " hidden")}
              data-index="-1"
              data-section={section}
            ></div>
            <div>{film.film}</div>
