...
export default class CurrentUserDropDown extends Component {
    ...
    return (
      <>
        <div className="current-user-dropdown">
          {!!autoRenewFilms.length && (
            <div className="icon" onClick={this.clickIcon.bind(this)}></div>
          )}
  ...
  renderModal() {
    const { autoRenewFilms, autoRenewModalOpen, spinner, autoRenewModalPage } =
      this.state;
    ...
    return (
      <>
        ...
        >
          <table className="auto-renew-films">
            ...
            <tbody>
              {autoRenewFilms.slice(startIndex, endIndex).map((film, index) => {
                const { title, endDate, autoRenewDaysNotice, autoRenewTerm } =
                  film;
                return (
                  <tr key={index}>
                    <td>{title}</td>
          ...
          />
          {autoRenewFilms.slice(startIndex).length > 10 && (
            <a
              className="next-page page-nav"
              onClick={this.clickNextPage.bind(this)}
            ></a>
          )}
          {autoRenewModalPage > 1 && (
            <a
              className="prev-page page-nav"
              onClick={this.clickPrevPage.bind(this)}
            ></a>
          )}
