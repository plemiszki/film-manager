...
describe 'dvd_reports', type: :feature do
  ...
  it 'calculates report' do
    date = Date.parse("1/1/#{Date.today.year}")
    settings = create(:setting)
    @aec_january_po = create(:purchase_order, customer_id: 1, order_date: date, ship_date: date, month: 1, number: 'JAN-AEC')
    ...
    Invoice.create_invoice_from_po(@aec_january_po, { sent_date: date })
    settings.update(next_dvd_invoice_number: settings.next_dvd_invoice_number + 1)
    @midwest_january_po = create(:purchase_order, customer_id: 2, order_date: date, ship_date: date, month: 1, number: 'JAN_MID')
    ...
    Invoice.create_invoice_from_po(@midwest_january_po, { sent_date: date })
    settings.update(next_dvd_invoice_number: settings.next_dvd_invoice_number + 1)
    @aec_february_po = create(:purchase_order, customer_id: 1, order_date: date, ship_date: date, month: 2, number: 'FEB-AEC')
    ...
    Invoice.create_invoice_from_po(@aec_february_po, { sent_date: date })
    settings.update(next_dvd_invoice_number: settings.next_dvd_invoice_number + 1)
    @midwest_february_po = create(:purchase_order, customer_id: 2, order_date: date, ship_date: date, month: 2, number: 'FEB-MID')
    ...
    Invoice.create_invoice_from_po(@midwest_february_po, { sent_date: date })
    settings.update(next_dvd_invoice_number: settings.next_dvd_invoice_number + 1)
