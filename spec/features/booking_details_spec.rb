...
describe 'booking_details', type: :feature do
  ...
  it 'displays the venue stripe ID if the email matches the venue, and the venue has a stripe ID' do
    @venue.update!(
  ...
  it 'it displays a notification if the email matches the venue, and the venue has no stripe ID' do
    @venue.update!(
      stripe_id: "",
    )
    @booking.update!(
    ...
    visit booking_path(@booking, as: $admin_user)
    expect(page).to have_content('This venue has no Stripe ID.')
  end

  it 'displays the booking stripe ID if the email does not match the venue, and the booking has a stripe ID' do
    @booking.update!(
      stripe_id: "bookingstripeid",
      use_stripe: true,
    )
  ...
  it 'displays the create stripe customer button if the email does not match the venue, and the booking has no stripe ID' do
    visit booking_path(@booking, as: $admin_user)
    expect(page).to have_content('Create Stripe Customer')
  end
